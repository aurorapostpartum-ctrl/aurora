...
import { createChecklistTemplate, updateChecklistTemplate } from '../../data/mockStore';
import { colors, radius, spacing } from '../../theme';
import type { ChecklistTemplate, TemplateVisibility } from '../../types/domain';
import {
  TRADE_OPTIONS,
  createDraftItem,
  createDraftSection,
  templateToDraftSections,
  type DraftItem,
  type DraftSection,
} from './checklistTemplateMeta';

export interface ChecklistTemplateBuilderProps {
  mode: 'create' | 'edit';
  template?: ChecklistTemplate;
  actorId: string;
}

export function ChecklistTemplateBuilder({ mode, template, actorId }: ChecklistTemplateBuilderProps) {
  ...
  const [sections, setSections] = useState<DraftSection[]>(() =>
    template ? templateToDraftSections(template) : [createDraftSection('')]
  );
  ...
  const addSection = () => setSections((prev) => [...prev, createDraftSection('')]);
  ...
  const addItem = (sectionKey: string) =>
    setSections((prev) => prev.map((s) => (s.key === sectionKey ? { ...s, items: [...s.items, createDraftItem()] } : s)));
  ...
  const handleSubmit = () => {
    ...
    const cleanedSections = sections
      .map((s) => ({ name: s.name, items: s.items.filter((i) => i.text.trim().length > 0) }))
    ...
    const payload = {
      ...
      sections: cleanedSections.map((s) => ({
        name: s.name,
        items: s.items.map((i) => ({ text: i.text, required: i.required, requiresPhoto: i.requiresPhoto, notes: i.notes })),
      })),
    };

    const result =
      mode === 'create'
        ? createChecklistTemplate({ ...payload, createdBy: actorId })
        : updateChecklistTemplate(template!.id, payload);

    setSubmitting(false);
    if (result) router.replace(`/(app)/checklist-template/${result.id}` as never);
  };
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View style={styles.inner}>
          <Text variant="largeTitle" style={styles.title}>
            {mode === 'create' ? 'New Checklist Template' : 'Edit Checklist Template'}
          </Text>
          <Text variant="body" color={colors.textSecondary} style={styles.subtitle}>
            Build a reusable checklist. Employees generate their own job-specific copy from it — this template
            itself never gets marked complete.
          </Text>

          <TextField
            label="Checklist Name"
            placeholder="e.g. Commercial Plumbing Final"
            value={name}
            onChangeText={(t) => {
              setName(t);
          ...
          {sections.map((section, sectionIndex) => (
            <Animated.View key={section.key} layout={LinearTransition.duration(220)} style={styles.section}>
              <View style={styles.sectionHeaderRow}>
                ...
                <TextField
                  label=""
                  placeholder="Section name (e.g. FIXTURES)"
                  value={section.name}
  ...
  return (
    <Animated.View layout={LinearTransition.duration(220)} style={styles.itemRow}>
      <View style={styles.itemMainRow}>
        ...
        <Pressable onPress={onRemove} hitSlop={8} style={styles.deleteButton} accessibilityLabel="Remove item">

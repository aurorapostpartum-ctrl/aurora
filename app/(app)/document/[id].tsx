...
import {
  toggleDocumentDownload,
  useIsDocumentDownloaded,
  useIsOnline,
} from "../../../src/data/offlineStore";
import {
  formatDate,
  getDocument,
  getJob,
...
import {
  ...
  SHEET_HEIGHT,
  SHEET_WIDTH,
} from "../../../src/features/documents/DrawingSheet";
import {
  CATEGORY_LABEL,
  currentRevision,
  revisionLabel,
...
function DocumentViewerContent() {
  ...
  const [viewingRevisionNumber, setViewingRevisionNumber] = useState<
    number | undefined
  >(undefined);
  ...
  const downloaded = useIsDocumentDownloaded(id ?? "");

  const revision = doc
    ? (doc.revisions.find((r) => r.revisionNumber === viewingRevisionNumber) ??
      currentRevision(doc))
    : undefined;
  ...
  const showReviewBanner =
    reviewState === "new_revision" || reviewState === "needs_review";
  const notAvailableOffline = !isOnline && !downloaded;
  ...
  const goBack = () => {
    router.push({
      pathname: "/(app)/job/[id]",
      params: { id: job.id, section: "documents" },
    });
  };

  const handleSelectRevision = (revisionNumber: number) => {
    setViewingRevisionNumber(
      revisionNumber === current.revisionNumber ? undefined : revisionNumber,
    );
  ...
  const handleNextPage = () =>
    setPage((p) => Math.min(revision.pageCount, p + 1));
  ...
  const handlePrevMatch = () => {
    if (searchMatches.length === 0) return;
    const next =
      (matchCursor - 1 + searchMatches.length) % searchMatches.length;
    setMatchCursor(next);
    setPage(searchMatches[next].page);
  };
    ...
    <Screen glow={false}>
      ...
      <View style={styles.titleWrap}>
        ...
        <View style={styles.badgeRow}>
          ...
          <Pressable
            ...
            disabled={!isOnline && !downloaded}
            style={[
              styles.downloadPill,
              downloaded && styles.downloadPillActive,
            ]}
            accessibilityLabel={
              downloaded ? "Remove offline download" : "Download for offline"
            }
          >
            <Ionicons
            ...
            <Text
              ...
              style={styles.downloadPillLabel}
            >
              {downloaded
                ? "Available Offline"
                : !isOnline
                  ? "Can't download offline"
      ...
      {notAvailableOffline ? (
        <EmptyState
          ...
          message="You're offline and this document hasn't been downloaded yet. Download it next time you're online to view it without a connection."
        />
      ) : (
        <>
          {showReviewBanner ? (
            <View style={styles.reviewBanner}>
              ...
              <View style={styles.reviewBannerText}>
                <Text
                  variant="headline"
                  color={colors.warning}
                  style={styles.reviewBannerTitle}
                >
                  {reviewState === "new_revision"
          ...
          {isViewingArchived ? (
            <View style={styles.archivedBanner}>
              ...
              >
                Viewing archived {revisionLabel(revision.revisionNumber)} — read
                only.
              </Text>
          ...
          >
            ...
            <ToolbarButton
              ...
              onPress={() => setUploadRevisionOpen(true)}
